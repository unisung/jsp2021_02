...
<%@page import="java.io.*"%><%@page import="java.sql.*"%><%@page import="java.util.*"%>
...
<sql:update dataSource="${dataSource}" var="resultSet">
		update webMember
		     set password=?, name=?, gender=?, birth=?, mail=?, phone=?, 
		          postcode=?, address=?, addressDetail=?, addressJibun=?, 
		          addressExtra=?, regist_day=?
         where id=?
...
<%
       /* 수정할 이미지 파일이 스트림으로 넘어왔을 때만 이미지 update 처리 */
      if(filename!=null){
